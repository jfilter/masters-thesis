...
def my_config():
    ...
    drop_mult=random.uniform(0.10, 0.20)
...
@ex.automain
def my_main(epochs, lr, drop_mult, exp_id, bs, based_on_model, layer_factor):
    torch.cuda.set_device(0)
    
    ex.info['path'] = EX_PA
    
    data_lm = TextLMDataBunch.load(EX_PA, bs=bs)
    learn = language_model_learner(data_lm, drop_mult=drop_mult).load(EX_PA/"models"/based_on_model, device="cuda:0")
