...
def setup_data(clas):
    UT = Path('~/data/ynacc_proc/replicate/threads_prev3')
    
    data_clas_train = pd.read_csv(UT/'train_cl.csv')
    data_clas_val = pd.read_csv(UT/'val_cl.csv')

    data_clas_train = data_clas_train[[clas, 'text_threads']]
    data_clas_val = data_clas_val[[clas, 'text_threads']]
    ...
    data_clas = TextClasDataBunch.from_df(EX_PA, data_clas_train, data_clas_val,
                                          vocab=learn_lm_vocab, bs=32, text_cols=['text_threads'], label_cols=[clas],)
    return data_clas
