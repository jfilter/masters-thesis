...
class YnaccProcessor(DataProcessor):
    ...
    def __init__(self, cls_idx):
        ...
        self.ynlabel = all_classes[cls_idx]

    def _create_examples(self, df):
        examples = []
        for i, row in df.iterrows():
            text_b = 'xxtoplevelcomment' if pd.isna(row['parentid']) else 'xxreplycomment'
            examples.append(InputExample(guid=i, text_a=row['text'],text_b=text_b,  label=row[self.ynlabel]))
            
        return examples

    def get_train_examples(self, data_dir):
        df = pd.read_csv('~/data/ynacc_proc/replicate/split/train.csv', usecols=['text', 'parentid',self.ynlabel])
        df = df.dropna(subset=[self.ynlabel])
        return self._create_examples(df)
    
    def get_dev_examples(self, data_dir):
        df = pd.read_csv('~/data/ynacc_proc/replicate/split/test.csv', usecols=['text', 'parentid', self.ynlabel])
        df = df.dropna(subset=[self.ynlabel])
        return self._create_examples(df)

    def get_labels(self):
        return [0, 1] if self.ynlabel != 'clsentiment' else [0, 1, 2, 3]
...
def main():
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        with open(os.path.join(args.output_dir, 'test_report.json'), "w") as writer:
