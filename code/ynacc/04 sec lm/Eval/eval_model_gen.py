...
def setup_data(clas):
    ...
    data_clas_train = pd.read_csv(split_path/'train_proc_fixed.csv')
    data_clas_val = pd.read_csv(split_path/'val_proc_fixed.csv')
...
for x in ['claudience','clpersuasive','clsentiment','clagreement','cldisagreement','clinformative','clmean','clcontroversial', 'cltopic']:
    torch.cuda.empty_cache()
#    if x in ['claudience','clpersuasive','clsentiment', 'clagreement','cldisagreement', 'clinformative','clmean']:
#        continue
    i += 1
