...
def cut_to_max_previous(x, max_prev):
    com_sta_pos = [m.start() for m in re.finditer('xx_comment_start', x)]
    if len(com_sta_pos) >  max_prev + 1:
        return x[com_sta_pos[-(max_prev + 1)]:]
    return x

def setup_data(clas, max_prev):
    ...
    for dat in [data_clas_train, data_clas_val]:
        dat['text_proc'] = dat['text_proc'].apply(lambda x: cut_to_max_previous(x, max_prev))
